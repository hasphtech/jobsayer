...
async function exchangeDropboxCode(code: string): Promise<TokenResponse> {
...
async function exchangeOnedriveCode(code: string): Promise<TokenResponse> {
...
export async function GET(req: NextRequest) {
  ...
  try {
    // Verify state payload matches current user
    const stateData = JSON.parse(Buffer.from(state, "base64url").toString()) as { userId: string; provider: string };
  ...
  try {
    ...
    if (provider === "google_drive") {
      ...
      tokens  = await exchangeDropboxCode(code);
      profile = await fetchDropboxProfile(tokens.access_token);
    } else {
      tokens  = await exchangeOnedriveCode(code);
    ...
    // Clear state cookie and redirect to vault with success signal
