...
interface Role {
  ...
  offers: number; daysOpen: number; fresh: boolean;
}
...
/* ── Mock Data ───────────────────────────────────────────────── */
const ROLES: Role[] = [
  { id: "r1", title: "Senior Backend Engineer", city: "Bangalore", type: "Hybrid", applicants: 47, bgvCleared: 12, interviews: 3, offers: 1, daysOpen: 8, fresh: true },
  { id: "r2", title: "Product Manager", city: "Mumbai", type: "Remote", applicants: 23, bgvCleared: 8, interviews: 5, offers: 2, daysOpen: 14, fresh: true },
  { id: "r3", title: "Frontend Engineer", city: "Hyderabad", type: "Onsite", applicants: 61, bgvCleared: 19, interviews: 7, offers: 0, daysOpen: 21, fresh: true },
  { id: "r4", title: "Data Scientist", city: "Bangalore", type: "Hybrid", applicants: 18, bgvCleared: 5, interviews: 2, offers: 0, daysOpen: 6, fresh: true },
];
...
export default function EmployerDashboard() {
  ...
  const TABS: { key: Tab; label: string; icon: string }[] = [
    ...
    { key: "pipeline",    label: "Pipeline",    icon: "ti-funnel" },
    { key: "candidates",  label: "Candidates",  icon: "ti-users" },
    { key: "bgv",         label: "BGV",         icon: "ti-shield-check" },
    { key: "team-tools",  label: "Team Tools",  icon: "ti-tools" },
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg, #08080c)", color: "var(--text1, #f0f0f0)", fontFamily: "var(--font-inter, Inter, sans-serif)" }}>
      ...
      {/* Top bar */}
      <div style={{ borderBottom: "1px solid var(--border)", padding: "0 24px", background: "var(--nav-bg, #0d0d14)", display: "flex", alignItems: "center", height: 52, gap: 16 }}>
        <Link href="/dashboard" style={{ color: "var(--accent)", fontSize: 13, fontWeight: 800, textDecoration: "none" }}>
          ← jobSayer
        </Link>
        <div style={{ width: 1, height: 18, background: "var(--border)" }} />
        <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text1)" }}>Employer Portal</span>
        <div style={{ flex: 1 }} />
        <div style={{ fontSize: 10, fontWeight: 800, padding: "3px 9px", borderRadius: 20, background: `${plan.color}18`, color: plan.color, border: `1px solid ${plan.color}30` }}>
          {plan.name} Plan
        </div>
        <Link href="/integrations" style={{ fontSize: 12, color: "var(--text3)", textDecoration: "none", display: "flex", alignItems: "center", gap: 5 }}>
          <i className="ti ti-plug-connected" style={{ fontSize: 13 }} /> Bots
        </Link>
      </div>
      ...
      <div style={{ display: "flex", minHeight: "calc(100vh - 52px)" }}>
        ...
        {/* Sidebar tabs */}
        <div style={{ width: 180, borderRight: "1px solid var(--border)", padding: "16px 10px", flexShrink: 0 }}>
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              style={{
                width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "9px 10px",
                borderRadius: 8, marginBottom: 2, fontSize: 12, fontWeight: tab === t.key ? 700 : 500,
                background: tab === t.key ? "rgba(99,102,241,.1)" : "transparent",
        ...
        <div style={{ flex: 1, padding: "24px", overflowY: "auto" }}>
          ...
          {tab === "overview" && (
            <div>
              <h2 style={{ fontSize: 18, fontWeight: 900, marginBottom: 4 }}>Hiring Overview</h2>
              <p style={{ fontSize: 12, color: "var(--text3)", marginBottom: 20 }}>Your hiring at a glance — last 30 days</p>
              ...
              <div style={{ marginBottom: 24 }}>
                <div style={{ fontSize: 12, fontWeight: 800, color: "var(--text3)", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 12 }}>Active Roles</div>
          ...
          {tab === "pipeline" && (
            <div>
              <h2 style={{ fontSize: 18, fontWeight: 900, marginBottom: 20 }}>Hiring Pipeline</h2>
                ...
                {(["new", "shortlisted", "interview", "offer"] as Candidate["stage"][]).map(stage => (
                  <div key={stage} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: "12px" }}>
                    <div style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".06em", color: "var(--text3)", marginBottom: 12 }}>
                      {stage === "new" ? "New" : stage === "shortlisted" ? "Shortlisted" : stage === "interview" ? "Interview" : "Offer"} ({CANDIDATES.filter(c => (stages[c.id] ?? c.stage) === stage).length})
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {CANDIDATES.filter(c => (stages[c.id] ?? c.stage) === stage).map(c => (
                        <div key={c.id} style={{ background: "rgba(255,255,255,.03)", border: "1px solid var(--border)", borderRadius: 8, padding: "10px" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                            <div style={{ width: 28, height: 28, borderRadius: "50%", background: "var(--accent)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 800, color: "#fff", flexShrink: 0 }}>{c.initials}</div>
                            <div>
          ...
          {tab === "candidates" && (
            <div>
              <h2 style={{ fontSize: 18, fontWeight: 900, marginBottom: 20 }}>Verified Candidates</h2>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {CANDIDATES.map(c => (
                  <div key={c.id} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 16px", display: "flex", alignItems: "center", gap: 14 }}>
                    <div style={{ width: 38, height: 38, borderRadius: "50%", background: "var(--accent)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: "#fff", flexShrink: 0 }}>{c.initials}</div>
                    <div style={{ flex: 1 }}>
          ...
          {tab === "bgv" && (
            <div>
              <h2 style={{ fontSize: 18, fontWeight: 900, marginBottom: 6 }}>BGV Management</h2>
              <p style={{ fontSize: 12, color: "var(--text3)", marginBottom: 20 }}>All verifications run at platform layer — no separate vendor contract needed.</p>
              ...
              {CANDIDATES.map(c => (
                <div key={c.id} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 16px", marginBottom: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: "var(--accent)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: "#fff" }}>{c.initials}</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text1)" }}>{c.name}</div>
                      <div style={{ fontSize: 11, color: "var(--text3)" }}>{c.role}</div>
                    </div>
                    <div style={{ fontSize: 10, fontWeight: 800, padding: "3px 8px", borderRadius: 10, background: c.bgv === "cleared" ? "rgba(34,197,94,.1)" : "rgba(251,191,36,.1)", color: c.bgv === "cleared" ? "#22c55e" : "#f59e0b", border: "1px solid " + (c.bgv === "cleared" ? "rgba(34,197,94,.25)" : "rgba(251,191,36,.25)") }}>
                      {c.bgv === "cleared" ? "All Cleared" : "In Progress"}
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {["Aadhaar", "PAN", "Address", "Education", "Employment", "Criminal"].map((check, i) => (
                      <span key={check} style={{ fontSize: 10, padding: "2px 8px", borderRadius: 6, background: (c.bgv === "cleared" || i < 4) ? "rgba(34,197,94,.1)" : "rgba(251,191,36,.1)", color: (c.bgv === "cleared" || i < 4) ? "#22c55e" : "#f59e0b", border: "1px solid " + ((c.bgv === "cleared" || i < 4) ? "rgba(34,197,94,.2)" : "rgba(251,191,36,.2)") }}>
          ...
          {/* ── TEAM TOOLS (White-label B2B2C) ── */}
          {tab === "team-tools" && (
            <div>
              <h2 style={{ fontSize: 18, fontWeight: 900, marginBottom: 6 }}>Team Career Tools</h2>
              <p style={{ fontSize: 13, color: "var(--text3)", marginBottom: 20 }}>Give your employees access to jobSayer's career growth tools as a company benefit — fully white-labelled with your brand.</p>
              ...
              <div style={{ background: "rgba(99,102,241,.08)", border: "1px solid rgba(99,102,241,.2)", borderRadius: 12, padding: "18px 20px", marginBottom: 24 }}>
                ...
                <p style={{ fontSize: 12, color: "var(--text3)", lineHeight: 1.65, margin: 0 }}>
                  Your employees get a branded portal at <strong style={{ color: "var(--accent)" }}>careers.yourcompany.com</strong> with resume builder, interview prep, career GPS, salary insights, and more. Powered by jobSayer, branded as your company. Available on Enterprise plan.
                </p>
              </div>
          ...
          {tab === "billing" && (
            <div>
              <h2 style={{ fontSize: 18, fontWeight: 900, marginBottom: 20 }}>Plans & Billing</h2>
